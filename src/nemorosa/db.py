...
import msgspec
from sqlalchemy import DDL, Boolean, ForeignKey, Index, Integer, String, delete, event, func, select, update
...
# Create trigger to auto-increment run_count on job_log updates
_trigger_update_run_count = DDL(
    """
    CREATE TRIGGER IF NOT EXISTS job_log_increment_run_count
    AFTER UPDATE OF last_run ON job_log
    FOR EACH ROW
    WHEN NEW.last_run != OLD.last_run
    BEGIN
        UPDATE job_log 
...
# Attach trigger creation to JobLog table
event.listen(JobLog.__table__, "after_create", _trigger_update_run_count)
...
class ClientTorrent(Base):
    ...
    def from_client_info(cls, info: "ClientTorrentInfo") -> "ClientTorrent":
        ...
        Example:
            >>> torrent_info = client.get_torrent_info(hash)
            >>> db_torrent = ClientTorrent.from_client_info(torrent_info)
            >>> await session.merge(db_torrent)
        """
        return cls(
            hash=info.hash,
            name=info.name,
...
class TorrentDatabase:
    ...
    async def update_job_run(self, job_name: str, last_run: datetime, next_run: datetime | None = None):
        ...
        """
        async with self.async_session_maker.begin() as session:
            # Use merge for upsert - trigger handles run_count automatically
            job_log = JobLog(job_name=job_name, last_run=last_run, next_run=next_run)
    ...
    async def delete_client_torrent(self, torrent_hash: str):
        """Delete torrent and its files from cache.

        Args:
            torrent_hash: Torrent hash to delete.
        """
        async with self.async_session_maker.begin() as session:
            # Files will be cascade deleted due to foreign key constraint
            stmt = delete(ClientTorrent).where(ClientTorrent.hash == torrent_hash)
            await session.execute(stmt)
    ...
    async def batch_save_client_torrents(self, torrents: list["ClientTorrentInfo"]):
        ...
        """
        async with self.async_session_maker.begin() as session:
            # Batch insert/update torrents
            for t in torrents:
                client_torrent = ClientTorrent.from_client_info(t)
                await session.merge(client_torrent)

            # Delete old files for all torrents
            if torrents:
                hashes = [t.hash for t in torrents]
                delete_stmt = delete(TorrentFile).where(TorrentFile.torrent_hash.in_(hashes))
                await session.execute(delete_stmt)
            ...
            for t in torrents:
                if t.files:
                    for file in t.files:
                        torrent_file = TorrentFile(torrent_hash=t.hash, file_path=file.name, file_size=file.size)
                        session.add(torrent_file)
    ...
    async def get_all_client_torrents_basic(self) -> dict[str, tuple[str, str]]:
        ...
        async with self.async_session_maker() as session:
            ...
            result = await session.execute(stmt)
            return {hash_val: (name, download_dir or "") for hash_val, name, download_dir in result.all()}
